...
class Pipeline(object):
    ...
    def main(self, argv):
        ...
        rsourcefile = ''
        try:
            opts, args = getopt.getopt(argv, "hd:r:", ["date=", "rfile="])
        except getopt.GetoptError:
            print 'main.py -d <date> -r <R source filename>'
            sys.exit(2)
        for opt, arg in opts:
            if opt == '-h':
                print 'main.py -d <date> -r <R source filename>'
        ...
        self.downloadforcountry("GIN", date)
        self.downloadforcountry("LBR", date)
        self.downloadforcountry("SLE", date)

        # windows specific at the moment
        # need to replace with a platform agnostic call to run R
        output = call(["C:\\Program Files\\R\\R-3.1.2\\bin\\R.exe", "--no-save", "--file=" + rsourcefile])

    def downloadforcountry(self, countryname, date):
